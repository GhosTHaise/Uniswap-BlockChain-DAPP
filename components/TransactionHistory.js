...
const style = {
    wrapper : `h-full text-white text-sm select-none w-screen overflow-x-hidden overflow-y-scroll flex-1 pt-14 flex items-center justify-end pb-5 mt-2  px-8`,
                ...
                {transactionHistory && transactionHistory?.map((transaction,index) => (
                    <div className={style.txHistoryItem} key={index}>
                        <div className={style.txDetails}>
                            <Image src={ethLogo} width={16} height={21} alt="eth" />
                            <span className={style.ethLogoSpacing}></span>{transaction.amount} ☰ sent to {' '}
                        ...
                        <div className={style.etherscanLink}>
                            <a href={`https://rinkeby.etherscan.io/tx/${transaction.txHash}`} target="_blank" rel='noreferrer' className={style.etherscanLink}>
                                View on etherscan
                                <FiArrowUpRight />
                            </a>
                        </div>
